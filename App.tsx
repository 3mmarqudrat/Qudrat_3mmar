...
export const MultiSelectDropdown: React.FC<{
    ...
    const toggleValue = (value: string) => {
        if (value === 'all') {
            ...
            const next = selectedValues.filter(v => v !== 'all');
    ...
    return (
        <div className="relative" ref={containerRef} dir="rtl">
            <label className="block text-xs font-bold text-text-muted mb-1 mr-1">{label}</label>
            <button
                type="button"
                onClick={(e) => { e.stopPropagation(); setIsOpen(!isOpen); }}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-md p-2 text-right text-sm flex justify-between items-center hover:border-zinc-500 transition-colors"
            >
                <span className="truncate">
                    {selectedValues.includes('all') ? 'الكل' : `${selectedValues.length} مختارة`}
                </span>
                <ChevronDownIcon className={`w-4 h-4 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </button>
            {isOpen && (
                <div className="absolute z-50 mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-md shadow-2xl max-h-60 overflow-y-auto custom-scrollbar">
                    {options.map((opt) => (
                        <label key={opt.value} className="flex items-center gap-2 p-2 hover:bg-zinc-700 cursor-pointer transition-colors border-b border-zinc-700/50 last:border-0">
                            <input
                                type="checkbox"
                                checked={selectedValues.includes(opt.value)}
                                onChange={() => toggleValue(opt.value)}
                                className="w-4 h-4 rounded border-zinc-600 text-primary bg-zinc-900 focus:ring-primary"
                            />
...
const ModeSelectionView: React.FC<{ onBack: () => void; onSelectSection: (section: Section, mode: 'training' | 'review') => void; userMode: 'training' | 'review'; settings: AppSettings; user: User; onLogout: () => void; }> = ({ onBack, onSelectSection, userMode, settings, user, onLogout }) => {
    ...
    return (
        <div className="bg-bg min-h-screen"><Header title={title} leftSlot={<button onClick={onBack} className="p-2 rounded-full hover:bg-zinc-700 transition-colors"><ArrowRightIcon className="w-6 h-6 text-text-muted rotate-180"/></button>} rightSlot={<UserMenu user={user} onLogout={onLogout} />} /><main className="container mx-auto p-4 md:p-8"><div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-bold text-text">{title}</h2><p className="mt-4 text-lg text-text-muted">{description}</p></div><div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto justify-center"><SectionCard title="الالقسم اللفظي" icon={<BookOpenIcon className="w-16 h-16 text-primary mx-auto"/>} onClick={() => onSelectSection('verbal', userMode)} description="التناظرة اللفظي، إكمال الجمل، والمزيد." enabled={isVerbalEnabled} /><SectionCard title="القسم الكمي" icon={<BarChartIcon className="w-16 h-16 text-primary mx-auto"/>} onClick={() => onSelectSection('quantitative', userMode)} description="الجبر، الهندسة، والإحصاء." enabled={isQuantitativeEnabled} /></div></main></div>
    );
};

const SectionView: React.FC<{ section: Section; data: AppData; onBack: () => void; onStartTest: (test: Test, bankKey?: string, categoryKey?: string) => void; onReviewAttempt: (attempt: TestAttempt) => void; headerLeftSlot?: React.ReactNode; headerRightSlot?: React.ReactNode; openBankKeys: Set<string>; onToggleBank: (bankKey: string) => void; selectedTestId: string | null; onSelectTest: (test: Test, bankKey?: string, categoryKey?: string) => void; onGoToReview: () => void; }> = ({ section, data, onBack, onStartTest, onReviewAttempt, headerLeftSlot, headerRightSlot, openBankKeys, onToggleBank, selectedTestId, onSelectTest, onGoToReview }) => {
    ...
    const getTestNumber = (name: string) => {
        ...
        const nMatch = n.match(/\d+/);
        return nMatch ? parseInt(nMatch[0], 10) : 0;
    };
...
export const App: React.FC = () => {
    ...
    const { data, isLoading: isDataLoading, addTest, addQuestionsToTest, deleteTests, updateQuestionAnswer, addAttemptToHistory, deleteUserData, addDelayedQuestionToReview, addSpecialLawQuestionToReview, reviewedQuestionIds, fetchTestQuestions } = useAppData(activeUser?.uid || null, isDevUser, isPreviewMode);
    
    const handleAddTestSync = useCallback((section: Section, testName: string, bankKey?: string, categoryKey?: string, sourceText?: string) => {
        return addTest(section, testName, bankKey, categoryKey, sourceText);
    }, [addTest]);

    const processor = useQuantitativeProcessor(handleAddTestSync as any, addQuestionsToTest);
    useEffect(() => { load(); }, [activeUser]);
    const load = async () => { if (activeUser?.uid) { const saved = await sessionService.loadSessionState(activeUser.uid); if (saved) { setOpenBankKeys(new Set(saved.openBankKeys || [])); setSelectedTestId(saved.selectedTestId || null); setUserMode(saved.userMode || null); if (saved.reviewFilters) setReviewFilters(saved.reviewFilters); if (saved.currentTest && saved.pageHistory && saved.pageHistory[saved.pageHistory.length - 1] === 'takeTest') { setPendingSession(saved); } else if (saved.currentTest) { sessionService.clearTestState(activeUser.uid); } } } }; 
    ...
    const clearTestState = () => { setCurrentTest(null); setUserAnswers([]); setElapsedTime(0); if (activeUser?.uid) { sessionService.clearTestState(activeUser.uid); } };
    const handleLogout = () => { authService.logout(); setCurrentUser(null); setPreviewUser(null); setPageHistory(['auth']); setShowLogoutConfirm(false); clearTestState(); };
    const handleTogglePreviewMode = () => { if (isDevUser) setIsPreviewMode(!isPreviewMode); };
    const handleStartTest = async (test: Test, bankKey?: string, categoryKey?: string, returnTo?: string) => { await fetchTestQuestions(test.id); clearTestState(); setSelectedTestId(test.id); setCurrentTest(test); setCurrentTestContext({ bankKey, categoryKey }); setReturnPath(returnTo || 'section'); if (!selectedSection) setSelectedSection(test.section === 'quantitative' || (test.questions && !!test.questions[0]?.questionImage) ? 'quantitative' : 'verbal'); navigate('takeTest'); };
    ...
    const handleFinishTest = (answers: UserAnswer[], duration: number) => {
        ...
        const score = answers.reduce((c, ua) => {
            const q = (currentTest.questions as Question[]).find(q => q.id === ua.questionId);
            return q && q.correctAnswer === ua.answer ? c + 1 : c;
        }, 0);
        const att: TestAttempt = { id: `att_${Date.now()}`, testId: currentTest.id, testName: currentTest.name, section: section, bankKey: currentTestContext.bankKey, categoryKey: currentTestContext.categoryKey, date: new Date().toISOString(), score, totalQuestions: (currentTest.questions as Question[]).length, answers, questions: currentTest.questions as Question[], durationSeconds: duration };
        
        addAttemptToHistory(att);

        setCurrentAttempt(att); setAttemptToReview(att); setSummaryReturnPage(returnPath || (currentTest.id.includes('review_') ? 'review' : (userMode === 'training' ? 'section' : 'review'))); clearTestState(); navigate('summary', true);
    };
    ...
    return (
        <>
            ...
            {pendingSession && <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm" dir="rtl"><div className="bg-surface rounded-lg p-8 m-4 max-w-md w-full text-center border border-border"><div className="mb-4"><ClockIcon className="w-16 h-16 mx-auto text-primary animate-pulse"/></div><h2 className="text-xl font-bold mb-4">استئناف الاختبار السابق</h2><p className="text-text-muted mb-6">هل تريد العودة إلى اختبار <strong>{pendingSession.currentTest?.name}</strong>؟</p><div className="flex justify-center gap-4"><button onClick={() => { clearTestState(); setPendingSession(null); }} className="px-6 py-2 bg-zinc-600 rounded-md">بدء جديد</button><button onClick={() => handleResumeTest(pendingSession)} className="px-6 py-2 bg-accent text-white rounded-md font-bold">إكمال الاختبار</button></div></div></div>}
             {showLogoutConfirm && <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 backdrop-blur-sm" dir="rtl"><div className="bg-surface rounded-lg p-8 m-4 max-sm w-full text-center border border-border"><h2 className="text-xl font-bold mb-4">تأكيد الخروج</h2><p className="text-text-muted mb-6">هل أنت متأكد؟</p><div className="flex justify-center gap-4"><button onClick={() => setShowLogoutConfirm(false)} className="px-6 py-2 bg-zinc-600 rounded-md">إلغاء</button><button onClick={handleLogout} className="px-6 py-2 bg-red-600 text-white rounded-md">خروج</button></div></div></div>}

            {page === 'home' && <HomeView username={activeUser.username} onSelectUserMode={(m) => { setUserMode(m); navigate(m === 'training' ? 'modeSelection' : 'review'); }} onLogout={() => setShowLogoutConfirm(true)} onGoToAdmin={() => navigate('admin')} onGoToSiteManagement={() => navigate('siteManagement')} onGoToVerbalManagement={() => navigate('verbalManagement')} onGoToQuantitativeManagement={() => navigate('quantitativeManagement')} isDevUser={isDevUser} isPreviewMode={isPreviewMode} onTogglePreviewMode={handleTogglePreviewMode} previewingUser={previewUser} onExitPreview={() => { setPreviewUser(null); navigate('admin'); }} />}
            {page === 'modeSelection' && userMode && <ModeSelectionView userMode={userMode} onBack={goBack} onSelectSection={(s, m) => { setSelectedSection(s); setUserMode(m); navigate(m === 'training' ? 'modeSelection' : 'review'); }} settings={settings} user={activeUser} onLogout={() => setShowLogoutConfirm(true)} />}
            {page === 'section' && selectedSection && <SectionView section={selectedSection} data={data} onBack={() => { goBack(); setSelectedTestId(null); }} onStartTest={handleStartTest} onReviewAttempt={(a) => { setAttemptToReview(a); setSummaryReturnPage(page); navigate('reviewAttempt'); }} headerLeftSlot={<div className="flex items-center gap-4"><button onClick={goBack} className="p-2 rounded-full hover:bg-zinc-700 transition-colors"><ArrowRightIcon className="w-6 h-6 text-text-muted rotate-180"/></button><button onClick={() => { setUserMode('review'); navigate('review'); }} className="px-3 py-2 text-sm font-bold rounded-md bg-zinc-700">المراجعة</button></div>} headerRightSlot={<UserMenu user={activeUser} onLogout={() => setShowLogoutConfirm(true)} />} openBankKeys={openBankKeys} onToggleBank={(k) => setOpenBankKeys(prev => { const n = new Set(prev); n.has(k) ? n.delete(k) : n.add(k); return n; })} selectedTestId={selectedTestId} onSelectTest={(t) => { setSelectedTestId(t.id); fetchTestQuestions(t.id); }} onGoToReview={() => navigate('review')} />}
            ...
            {page === 'review' && <ReviewView data={data} onBack={goBack} onStartReviewTest={(t) => handleStartTest(t, undefined, undefined, 'review')} />}
            ...
            {page === 'verbalManagement' && <VerbalManagementView data={data} onBack={goBack} onAddTest={handleAddTestSync as any} onAddQuestionsToTest={addQuestionsToTest} onDeleteTest={deleteTests} onUpdateQuestionAnswer={updateQuestionAnswer} />}
            {page === 'quantitativeManagement' && <QuantitativeManagementView onBack={goBack} onStartTest={(t, r) => handleStartTest(t, undefined, undefined, r || 'quantitativeManagement')} data={data} onAddTest={handleAddTestSync as any} onAddQuestionsToTest={addQuestionsToTest} onUpdateQuestionAnswer={updateQuestionAnswer} onDeleteTests={deleteTests} processorQueue={processor.queue} isProcessorWorking={processor.isProcessing} onAddFilesToQueue={processor.addFilesToQueue} onClearCompleted={processor.clearCompleted} onStopProcessing={processor.cancelAll} onSelectTest={(tid) => fetchTestQuestions(tid)} />}
        </>
    );
};
